    ...
    return MainLayout(
      ...
      child: Container(
        ...
        child: Stack(children: [
          ...
          Obx(() => ctrl.isLoading.value ? _LoadingOverlay() : const SizedBox()),
          Obx(() => ctrl.showAlertPopup.value
              ? _AlertPopup(controller: ctrl) : const SizedBox()),
          Obx(() => ctrl.showChangePopup.value
              ? _ChangePopup(controller: ctrl) : const SizedBox()),
          Obx(() => ctrl.showPrintPopup.value
              ? _InfoPopup(message: ctrl.popupMessage.value) : const SizedBox()),
          Obx(() => ctrl.showWaitPopup.value
              ? _WaitPopup(seconds: ctrl.afterFlashSeconds.value) : const SizedBox()),
        ]),
      ),
    );
  }
...
class _TableHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        _TH(w: 52,  label: '#'),            const _Dv(),
        _TH(w: 58,  label: 'State'),        const _Dv(),
        Expanded(flex: 5, child: _TH(label: 'Cal Id')),     const _Dv(),
        Expanded(flex: 4, child: _TH(label: 'ECU Sr No.')), const _Dv(),
        Expanded(flex: 3, child: _TH(label: 'SW Ver.')),    const _Dv(),
        Expanded(flex: 3, child: _TH(label: 'CVN')),        const _Dv(),
        Expanded(flex: 3, child: _TH(label: 'Part No.')),   const _Dv(),
        _TH(w: 62,  label: 'Dongle'),       const _Dv(),
        _TH(w: 58,  label: 'ECU'),          const _Dv(),
        Expanded(flex: 5, child: _TH(label: 'Progress')),   const _Dv(),
        _TH(w: 62,  label: 'Time'),         const _Dv(),
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            SizedBox(width: 58, child: Column(children: [
              Expanded(child: Center(
                child: _StatePill('Before', _w40, _w10))),
              Container(height: 0.8, color: _border),
              Expanded(child: Center(
                child: _StatePill('After', _orange,
        ...
        children: [
          ...
          Row(
            ...
            children: ['0', '25%', '50%', '75%', '100%']
              .map((t) => Text(t,
                style: const TextStyle(fontSize: 7, color: _w20)))
              .toList()),
        ],
      ),
    );
...
class _MiniBtn extends StatelessWidget {
  ...
  const _MiniBtn({required this.label, required this.icon,
...
class _HwIndicator extends StatelessWidget {
  ...
  final String label;
  const _HwIndicator({
    required this.on, required this.icon, required this.label});
  @override
  Widget build(BuildContext context) {
      ...
      children: [
        AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: on
                ? [BoxShadow(color: glow, blurRadius: 8)] : null),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: const Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          SizedBox(width: 6),
          Text('Find Dongle', style: TextStyle(
...
class _DongleScanDialogState extends State<_DongleScanDialog> {
    ...
    try {
      ...
      for (int start = 1; start <= 254; start += 20) {
        ...
        if (mounted) {
          setState(() {
            _progress = end / 254;
            _status = 'Scanning... ${(_progress * 100).toInt()}%'
                '  —  Found: ${found.length}';
          });
        }
      }
  ...
  void _selectIP(String ip) {
    ...
    Get.snackbar('✅ Dongle Found', 'IP: $ip  ·  Port 6888',
      backgroundColor: _pass, colorText: _w,
          ...
          Container(
            ...
            child: Row(children: [
              ...
              const Expanded(child: Text('Scanning for Dongles',
                style: TextStyle(color: _w, fontSize: 15,
                  fontWeight: FontWeight.bold))),
              if (!_scanning)
                GestureDetector(
                  onTap: () => Navigator.of(context).pop(),
                  child: const Icon(
                    Icons.close_rounded, color: _w, size: 20)),
            ])),
                  ...
                  if (_foundIPs.isEmpty)
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(8),
                        border: Border.all(
                          color: _fail.withOpacity(0.3))),
                      child: const Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('No dongles found on port 6888.',
                            style: TextStyle(color: _fail,
                              fontSize: 12, fontWeight: FontWeight.bold)),
                          SizedBox(height: 8),
                          Text('• Dongle powered ON (LED blinking)',
                            style: TextStyle(color: _w40, fontSize: 11)),
                          Text('• Same WiFi network as laptop',
                            style: TextStyle(color: _w40, fontSize: 11)),
                          Text('• Port 6888 not blocked by firewall',
                            style: TextStyle(color: _w40, fontSize: 11)),
                        ]))
                  else ...[
                    Text('${_foundIPs.length} Dongle(s) — tap to copy:',
                      style: const TextStyle(color: _pass,
                        fontSize: 12, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                          ...
                          Expanded(child: Column(
                            ...
                            children: [
                              Text(ip, style: const TextStyle(
                                color: _w, fontSize: 15,
                                fontWeight: FontWeight.bold,
                                fontFamily: 'monospace')),
                              const Text('Port 6888 · Responding ✅',
                                style: TextStyle(
                                  color: _w40, fontSize: 10)),
                            ])),
  ...
  Widget build(BuildContext context) {
    return _Shell(
      ...
      child: Column(children: [
        ...
        Row(children: [
          Expanded(child: _Btn(
            label: 'Reflash', onTap: controller.onReflash)),
          const SizedBox(width: 10),
          Expanded(child: _Btn(
            label: 'Change ECU',
            onTap: controller.onChangeECU, outlined: true)),
        ]),
      ]));
  }
}
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: _gOrange,
              boxShadow: [BoxShadow(
                color: _orangeGlow, blurRadius: 16)]),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: outlined
              ? Border.all(color: _orange, width: 1.5) : null,
          borderRadius: BorderRadius.circular(9),
